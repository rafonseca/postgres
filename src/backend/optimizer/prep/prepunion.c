						...
						List **pTargetList)
{
	 ...
	 */
	if (root->parent_root &&
		root->parent_root->partial_agg_pushdown != NULL)
	{
		PartialAggPushdownInfo *pushdown = root->parent_root->partial_agg_pushdown;
		ListCell   *lc_pos;
		ListCell   *lc_trans;
		ListCell   *lc_combine;
		ListCell   *lc_agg;
				...
				lc_agg, pushdown->outerAggrefs)
		{
			...
			/* Get the input Var from recursive term's tlist */
			rtle = (TargetEntry *) list_nth(rpath_tlist, colpos - 1);
			input_var_r = copyObject((Var *) rtle->expr);
			...
			/* Input Var type is transition type (from worktable) */
			input_var_r->vartype = transtype;
			arg_tle_r = makeTargetEntry((Expr *) input_var_r, 1, NULL, false);
			...
			final_agg->aggsplit = AGGSPLIT_FINAL_DESERIAL;
		...
		/*
		 * Note: setOp->colTypes was already updated by SS_process_ctes()
		 * before calling subquery_planner(), so we don't need to update
		 * it here.  generate_append_tlist will see the correct types.
		 */
		...
		/*
		 * Wrap the recursive term (rpath) with HashAggregate FINAL_DESERIAL.
		 */
