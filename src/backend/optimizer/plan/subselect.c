										  ...
										  CommonTableExpr *cte)
{
	...
	/* Check the CTE subquery: must be UNION ALL */
	...
	{
		elog(DEBUG1, "partial_agg_pushdown: not UNION ALL (op=%d, all=%d)",
			 setOp->op, setOp->all);
		return NULL;
	}
	...
	foreach(lc, parse->targetList)
	{
		...
		/* Must NOT have finalfn */
		if (OidIsValid(aggform->aggfinalfn))
		{
			elog(DEBUG1, "partial_agg_pushdown: has finalfn");
			ReleaseSysCache(aggTuple);
			return NULL;
		}
	...
	/*
	 * Verify that in the recursive term, each aggregated CTE column is a
	 * simple Var reference to the self-reference RTE (worktable).
	 */
	foreach(lc, aggColPositions)
	{
		int			colpos = lfirst_int(lc);
		TargetEntry *rec_tle;
		Var		   *rec_var;

		/* Get the target entry in the recursive term for this column position */
		rec_tle = (TargetEntry *) list_nth(recursive_term->targetList,
										   colpos - 1);
		if (rec_tle == NULL || !IsA(rec_tle->expr, Var))
			return NULL;

		rec_var = (Var *) rec_tle->expr;
									   ...
									   PartialAggPushdownInfo *info)
{
	...
	/* Mark the outer query to use FINAL_DESERIAL for its Agg node */
	root->cte_pushed_aggsplit = AGGSPLIT_FINAL_DESERIAL;
}
...
SS_process_ctes(PlannerInfo *root)
{
	...
	foreach(lc, root->parse->cteList)
	{
		...
		if (cte->cterecursive)
		{
			PartialAggPushdownInfo *pushdown;

			pushdown = can_pushdown_partial_agg_to_recursive_cte(root, cte);
			if (pushdown != NULL)
			{
				elog(DEBUG1, "partial_agg_pushdown: entering pushdown block");
				root->partial_agg_pushdown = pushdown;

				/*
				 * Update the CTE subquery to reflect the new column types
				 * for aggregated columns.  We must update:
				 * 1. SetOperationStmt colTypes (so UNION type checking works)
				 * 2. Self-reference RTE coltypes (for WorkTableScan)
				 * 3. Var nodes in the recursive term referencing the
				 ...
				 */
				{
					...
					/* 1. Update SetOperationStmt colTypes */
							...
							lc_trans, pushdown->aggTransTypes)
					{
						...
						cell = list_nth_cell(sop->colTypes, colpos - 1);
		 ...
		 */
		if (root->partial_agg_pushdown != NULL)
		{
			adjust_outer_query_for_partial_agg_cte(root, cte,
												   root->partial_agg_pushdown);
